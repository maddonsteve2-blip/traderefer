...
export function EditableGallery({ initialImages, businessName }: { initialImages: string[]; businessName: string }) {
    const context = useEditableProfileOptional();
    const rawImages = context?.isOwner ? (context.fields?.photo_urls || []) : initialImages;
    // Deduplicate images to prevent rendering duplicates
    const images = rawImages ? [...new Set(rawImages.filter(url => url && url.length > 5))] : [];
        ...
        <>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {images.map((url, index) => (
                    <div 
                        key={`${url}-${index}`} 
                        className="aspect-square rounded-xl overflow-hidden bg-zinc-100 group relative cursor-pointer"
                        onClick={() => setLightboxIndex(index)}
                    >
                        <img src={url} alt={`${businessName} work`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                        <div className="absolute inset-0 transition-opacity bg-black/20 opacity-0 group-hover:opacity-100 flex items-center justify-center">
            ...
            {lightboxIndex !== null && (
                ...
                >
                    ...
                    <img 
                        ...
                        alt={`${businessName} work ${lightboxIndex + 1}`}

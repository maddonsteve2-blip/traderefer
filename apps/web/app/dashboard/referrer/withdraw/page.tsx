                    ...
                    <div className="lg:col-span-8 space-y-5">
                        ...
                        <div className="bg-zinc-900 rounded-[32px] p-5 md:p-8 text-white relative overflow-hidden shadow-2xl">
                            ...
                            <div className="relative z-10 pt-2">
                                <p className="font-black text-zinc-400 uppercase tracking-widest mb-2 text-base">
                                    Pending Reward Balance
                                </p>
                                <p className="font-black text-white tracking-tighter leading-none mb-3 text-5xl">
                                ...
                                {!isReady && (
                                    <div className="mb-6">
                                        <div className="flex justify-between mb-1.5">
                                            <span className="font-bold text-zinc-400 text-base">Progress to minimum claim</span>
                                            <span className="font-bold text-orange-400 text-base">${pendingBalance.toFixed(2)} / $25.00</span>
                                        </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                                        <p className="font-black text-zinc-400 uppercase tracking-widest mb-1 text-lg">Total Earned</p>
                                        <p className="font-black text-zinc-100 text-2xl">${totalEarned.toFixed(2)}</p>
                                    </div>
                                    <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                        ...
                        <div className="bg-white rounded-2xl border border-zinc-200 p-6 shadow-sm">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                                <p className="font-black text-zinc-900 text-xl">One Card. 400+ Places to Spend It.</p>
                                <Link href="/rewards" className="font-bold text-orange-500 hover:text-orange-600 underline underline-offset-2 text-lg">
                                    See all →
                                </Link>
                            </div>
                            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
                                {CATALOG_BRANDS.map((brand) => (
                                    ...
                                    >
                                        <span className="font-black leading-tight text-center text-xs" style={{ color: brand.text }}>
                                            {brand.name}
                                        </span>
                                    </div>
                                ))}
                            </div>
                            <p className="text-zinc-400 font-medium mt-3 text-base">
                        ...
                        {/* HOW REWARDS WORK */}
                        <div className="bg-white rounded-2xl border border-zinc-200 p-6 shadow-sm">
                            <h3 className="font-black text-zinc-900 mb-4 text-xl">How Rewards Work</h3>
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={title} className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <p className="font-black text-zinc-900 text-lg">{title}</p>
                                            <p className="text-zinc-500 font-medium text-base">{desc}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        {prezzeePayouts.length > 0 && (
                            <div className="bg-white rounded-2xl border border-zinc-200 p-6 shadow-sm">
                                <h3 className="font-black text-zinc-900 mb-4 flex items-center gap-2 text-xl">
                                    ...
                                    {prezzeePayouts.slice(0, 5).map((p: { id: string; amount_cents: number; created_at: string; destination_email?: string }) => (
                                        <div key={p.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 py-3 border-b border-zinc-50 last:border-0">
                                            <div className="min-w-0">
                                                <p className="font-black text-zinc-900 text-lg">${(p.amount_cents / 100).toFixed(2)} Prezzee Swap</p>
                                                <p className="text-zinc-400 font-medium break-all text-base">
                                                    {new Date(p.created_at).toLocaleDateString('en-AU')} · {p.destination_email || "gift card"}
                                                </p>
                                            </div>
                                            <span className="font-bold text-green-600 bg-green-50 border border-green-200 px-3 py-1.5 rounded-full text-base">Issued</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="lg:col-span-4 space-y-5 lg:sticky lg:top-24 self-start">
                        ...
                        <div className="bg-white rounded-2xl border border-zinc-200 p-6 shadow-sm">
                            ...
                            <div className="text-center py-4">
                                <p className={`font-black tracking-tight ${qualityColor} text-6xl leading-none`}>
                                    {qualityScore}
                                </p>
                                <p className="font-black text-zinc-400 uppercase tracking-widest mt-1 text-base">
                                    out of 100
                                </p>
                            </div>
                            ...
                            <p className={`font-black text-center leading-snug ${qualityColor} text-xl`}>
                                {qualityScore >= 70
                            ...
                            <div className="space-y-3">
                                {[
                                    ...
                                    <div key={label} className="flex items-center justify-between py-3 border-b border-zinc-50 last:border-0">
                                        <span className="font-bold text-zinc-500 text-base">{label}</span>
                                        <span className={`font-black ${highlight ? "text-green-600" : "text-zinc-900"} text-xl`}>{value}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-zinc-900 rounded-2xl overflow-hidden shadow-xl">
                            ...
                            <div className="p-4 text-center">
                                <p className="font-black text-white text-lg">The Prezzee Smart Card</p>
                                <p className="text-zinc-400 font-medium text-base">One card. Endless choice.</p>
                                <Link href="/rewards" className="inline-block mt-3 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-bold transition-colors text-base">
                                    View all 400+ brands →
                                </Link>
                            </div>
                        </div>
